...
meso_df[map_var + '_barnes'] = map(barnes_crossval, xrange(len(meso_df)))
...
output_dir = 'output/xval_barnes/'
output_fname = 'meso_xval_%dcm_%s.csv'
meso_df.index.name = 'STID'
meso_df = meso_df.rename(columns = {'%s_%d' % (map_var, depth): 'vwc'})
meso_df[['vwc', 'vwc_barnes']].to_csv(output_dir + output_fname % (depth, date_str))
